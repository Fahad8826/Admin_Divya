...
Future<List<Map<String, dynamic>>> _getFilteredOrdersDataForReport() async {
  try {
    ...
    if (startDate.value != null && endDate.value != null) {
      ...
      query = query.where(
        'deliveryDate',
        isLessThanOrEqualTo: Timestamp.fromDate(endDate.value!.add(const Duration(days: 1))),
      );
    }

    final QuerySnapshot<Map<String, dynamic>> orderSnapshot = await query.get();
  ...
  //   return file;
  // }
  Future<File> _generateOrdersExcelFile(List<Map<String, dynamic>> ordersData) async {
  final excel = Excel.createExcel();
  final Sheet sheet = excel['Delivered Orders'];

  // Add filter description
  final filterDescription = [
    if (salespersonFilter.value.isNotEmpty) 'Salesperson: ${salespersonFilter.value}',
  ...
  if (filterDescription.isNotEmpty) {
    var filterCell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0));
  ...
  // Set column widths
  final columnWidths = [
    15.0, 25.0, 15.0, 15.0, 30.0, 20.0, 10.0, 15.0, 15.0, 15.0,
    15.0, 20.0, 20.0, 20.0, 25.0, 30.0,
  ];

  for (int i = 0; i < headers.length; i++) {
    sheet.setColumnWidth(i, columnWidths[i]);
    var cell = sheet.cell(CellIndex.indexByColumnRow(
        columnIndex: i, rowIndex: filterDescription.isNotEmpty ? 1 : 0));
    cell.value = TextCellValue(headers[i]);
    cell.cellStyle = CellStyle(
      bold: true,
      fontSize: 12,
  ...
  for (int rowIndex = 0; rowIndex < ordersData.length; rowIndex++) {
    ...
    final rowData = [
      ...
      (order['createdAt'] as DateTime?) != null ? dateFormat.format(order['createdAt']) : 'N/A',
      (order['deliveryDate'] as DateTime?) != null ? dateFormat.format(order['deliveryDate']) : 'N/A',
      (order['followUpDate'] as DateTime?) != null ? dateFormat.format(order['followUpDate']) : 'N/A',
    ...
    for (int colIndex = 0; colIndex < rowData.length; colIndex++) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(
          columnIndex: colIndex, rowIndex: rowIndex + (filterDescription.isNotEmpty ? 2 : 1)));
      cell.value = TextCellValue(rowData[colIndex]);
  ...
  // Add summary row
  final summaryRow = ordersData.length + (filterDescription.isNotEmpty ? 3 : 2);
  var summaryCell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: summaryRow));
  summaryCell.value = TextCellValue('Total Orders: ${ordersData.length}');
  summaryCell.cellStyle = CellStyle(
    bold: true,
    fontSize: 12,
  ...
  // Add timestamp
  var timestampCell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: summaryRow + 1));
  timestampCell.value = TextCellValue('Generated on: ${dateFormat.format(DateTime.now().toLocal())}');
  timestampCell.cellStyle = CellStyle(italic: true, fontSize: 10);

  // Save to temporary directory for sharing consistency
  final outputDir = await getTemporaryDirectory();
  final file = File('${outputDir.path}/delivered_orders_${DateTime.now().millisecondsSinceEpoch}.xlsx');
  final bytes = excel.encode();
  if (bytes != null) {
    await file.writeAsBytes(bytes);
  } else {
...
Future<void> downloadAllOrdersDataAsPDF(BuildContext context) async {
  ...
  try {
    ...
    final outputDir = await getApplicationDocumentsDirectory();
    final fileName = 'delivered_orders_${DateTime.now().millisecondsSinceEpoch}.pdf';
    final file = File('${outputDir.path}/$fileName');
    await file.writeAsBytes(await pdf.save());
...
Future<void> downloadSingleSalesmanAsPDF(BuildContext context, String salesmanName) async {
  if (isLoading.value) return;
...
Future<void> downloadSingleSalesmanAsExcel(BuildContext context, String salesmanName) async {
  if (isLoading.value) return;
  salespersonFilter.value = salesmanName;
...
Future<void> shareAllOrdersDataAsPDF(BuildContext context) async {
  ...
  try {
    ...
    await Share.shareXFiles(
      ...
      subject: 'Delivered Orders Report - ${dateFormat.format(DateTime.now().toLocal())}',
    );
...
Future<void> shareAllOrdersDataAsExcel(BuildContext context) async {
  ...
  try {
    ...
    await Share.shareXFiles(
      [XFile(file.path)],
      text: 'Delivered Orders Report',
      subject: 'Delivered Orders Report - ${DateFormat('dd-MMM-yyyy HH:mm').format(DateTime.now().toLocal())}',
    );
